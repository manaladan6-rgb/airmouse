...
Task ID: 31-docs
Agent: docs subagent
Task: v15 documentation set

Work Log:
- Read worklog.md (v12→v15 plan + all prior stage logs), README/CHANGELOG/VERIFICATION_REPORT (format/tone) and docs/ inventory first
- RE-VERIFIED key facts against code before writing: ran the FULL suite → 1056 passed / 0 failed / 0 skipped (Python 3.12.14, ~12.3 s); collected per-file counts via pytest --collect-only → test_v12 39, test_v13 40, test_v13_5 19, test_v14 33, test_v14_5 34, test_v15 75, test_hardening_v15 30 = 270 new (786 preserved)
- Read all 17 new module sources (docstrings, constants, schemas, method tables): twin (14 TwinCategory values, MAX_FACTS 2000, decay half-life 720 h, scrubbing regexes), world_model_temporal (MAX_HISTORY 128, frozen dataclasses, expect/mismatches), goals (pattern tables, execution_allowed always False), tasks (bounds 100×64×8, RetryPolicy ≤3, checkpoints), skills (MIN_OCCURRENCES 3, MIN_CONFIDENCE 0.6, _signature_for ignores coordinates), recovery2 (7 strategies, 14 FailureKind, MAX_ROUNDS 6, PERMISSION_DENIED→REQUEST_HUMAN only, MALFORMED→GIVE_UP), target_resolver (7-provider DEFAULT_RESOLUTION_ORDER, allow_coordinate_fallback), aip (_SCHEMAS 12, MsgType 19 = 18 conversation + STATUS, negotiate_version same-major, MAX_MESSAGE_BYTES 256 KB), agent_sdk (AipEndpoint fail-closed to ask ⇒ permission_denied), agents (MAX_AGENTS 32, leases TTL 30/300, holder-keeps rule, emergency_stop_all), permissions (ControlLevel 0..5, 12 PERMISSION_KEYS, 6 decisions), ditm (proposal fields, 5 user verbs), onboarding (6 entry choices, 8 ACCESSIBILITY_MODES), licensing (7 tiers, CORE_FEATURES 21), marketplace (_VALID_KEYS fail-closed, semver, trusted_by_human), simulator, failure_injection (12 FAILURE_CLASSES), explain (6 traces); plus agent-core/ (3 files ≈10 KB, stdio:// transport) and agent-sdk-js/ (dependency-free UMD)
- Verified HUD badge code (__main__.py 232-248: AGENT:/TASK:/CONFIRM?/RECOVER: per §33) and the 11 v15 CLI subcommands (§32)
- Counted the §30 fuzz corpus exactly: MALICIOUS_STRINGS = 23 items (task brief said 24; honesty rule → documented 23), 11 fuzz test surfaces confirmed
- WROTE airmouse_pkg/docs/V15_ARCHITECTURE.md (NEW, 361 lines): shared pipeline diagram PERCEPTION→WORLD MODEL→INTENT→TASK→PERMISSION→ACTION→VERIFICATION→RECOVERY→LEARNING; module map table with chapter references and real LOC; per-module sections §2-§33; honest-scope section
- WROTE airmouse_pkg/docs/AIP_SPEC.md (NEW, 365 lines): envelope field table; 18 conversation types + STATUS; all 12 schemas copied field-by-field from aip._SCHEMAS; negotiation rule; size limits; 8 fail-closed validation rules; EXECUTE request/response JSON pair (success + permission_denied) matching AipMessage.to_dict()/parse_message and test_v14_5 flows; in-process + stdio transports; conformance checklist
- WROTE airmouse_pkg/docs/AGENT_SDK.md (NEW, 205 lines): §10 example; primitives tables for agent_sdk.AirMouse / airmouse_agent / JS SDK; what the SDK hides; what it cannot do (no bypass pathway); performance budgets
- WROTE airmouse_pkg/docs/MULTI_AGENT.md (NEW, 199 lines): §12 registration/leases/conflict/handoff/data-only messaging/override/e-stop; Research→Writing→Browser walkthrough from test_v15.py; §13 shared-layer contract table; §14 hierarchy + §15 keys/decisions
- WROTE airmouse_pkg/docs/SKILLS.md (NEW, 214 lines): discovery→proposal→approval pipeline diagram; coordinate-fallback flag rule; skill schema JSON; library lifecycle; §20 manifest field table + trust boundaries + lifecycle
- WROTE airmouse_pkg/docs/DEVELOPER_GUIDE.md (NEW, 214 lines): install → first action via BOTH Python paths (in-process SDK + standalone airmouse-agent-core); simulator as fake computer (§25/§26); failure injection; CLI; budgets; safety rules
- WROTE airmouse_pkg/docs/USER_GUIDE.md (NEW, 146 lines): DO IT WITH ME walkthrough (propose→approve→START/EDIT/PAUSE/STOP/CHANGE_DIRECTION→progress→corrections); one-choice onboarding table; HUD badge meanings incl. AGENT: = an AI agent is controlling the computer; accessibility modes; privacy controls (twin reset/export/forget, telemetry structurally OFF, telemetry_default fix noted)
- UPDATED airmouse_pkg/README.md (246 lines): title → v15.0.0, hero extended, "What's new in v12→v15" table (one row per version: v12 twin+temporal, v12.5 temporal model, v13 goals+tasks, v13.5 skills, v14 recovery+target resolution, v14.5 AIP+SDKs+agent-core, v15 agents/permissions/DiTM/onboarding/licensing/marketplace/simulator/failure-injection/explainability/CLI/HUD) + §10 agent-SDK example; ALL existing v11.5/v10 content preserved below (wheel name updated 11.5.0→15.0.0)
- UPDATED airmouse_pkg/CHANGELOG.md (679 lines): v15.0.0 entry on top (Added per version / Changed / Security incl. §23 audit + §30 fuzz + telemetry fix / Fixed incl. telemetry_default defect / Verification with exact numbers); all v11.5/v10/v9/v5 entries intact
- UPDATED airmouse_pkg/VERIFICATION_REPORT.md (514 lines): v15 section on top with the exact measured numbers (786 baseline, 1056 final ~12 s, per-file new-test table, milestone tags 825→1056), SIMULATION vs PHYSICAL table (ALL v12-v15 = SIMULATION-VERIFIED ONLY; webcam/mic/gaze/RF/CDP/ASR NOT PHYSICALLY VERIFIED; simulator IS the computer model; AIP protocol-level in-process + stdio), security audit results, performance budgets; v11.5 + v10 reports preserved below
- UTF-8 validated for all 10 files; no Python code or tests modified; no git commands run
- Honesty corrections during self-review: 24→23 hostile fuzz strings (code-measured); "18 message types" documented as 18 conversation types + STATUS (19 enum values); test_v15.py reported as measured 75 (not "~100")
