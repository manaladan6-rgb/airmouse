...
class IntentEngine:
    ...
    def process(
        ...
        with self._lock:
            ...
            #    SAFETY CONTRACT: confirmation patterns are designed for gaze
            #    targeting (LOOK -> TARGET -> PINCH -> CLICK), so these
            #    intents are always gaze-class — the safety gate demands
            #    gaze-level confidence even if this decision's point came
            #    from another modality.  Hand-only clicking (no gaze) flows
            #    through the gesture-direct rule instead.
            click_emitted = False
